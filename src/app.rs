...
impl App {
    ...
    pub fn handle_input_event(&mut self, ev: Event<KeyEvent>) -> Result<()> {
        match ev {
    ...
    fn handle_input(&mut self, ev: KeyEvent) -> Result<()> {
    ...
    pub fn display_error(&mut self, error: anyhow::Error) {
        self.error_popup.set_message(error.to_string());
